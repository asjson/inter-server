...
RUN mkdir  $HOME/.vnc
RUN echo 'asjson' | vncpasswd -f > $HOME/.vnc/passwd
RUN echo '/bin/env  MOZ_FAKE_NO_SANDBOX=1  dbus-launch xfce4-session'  > $HOME/.vnc/xstartup
RUN chmod 600 $HOME/.vnc/passwd
RUN chmod 755 $HOME/.vnc/xstartup
RUN echo 'whoami ' >>/asjson.sh
RUN echo 'cd ' >>/asjson.sh
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/asjson.sh
RUN echo 'cd /noVNC-1.2.0' >>/asjson.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/asjson.sh
RUN chmod 755 /asjson.sh
EXPOSE 8900
CMD  /asjson.sh
